...
const i18nSource = fs.readFileSync(path.join(pluginDir, 'assets/js/expert-wage-i18n.js'), 'utf8');
const engineSource = fs.readFileSync(path.join(pluginDir, 'assets/js/expert-wage-engine.js'), 'utf8');
...
function createRuntime(translations) {
    ...
    vm.runInNewContext(i18nSource, context, { filename: 'expert-wage-i18n.js' });
    vm.runInNewContext(engineSource, context, { filename: 'expert-wage-engine.js' });
    return context;
}

function localeMessages(locale) {
    const filename = fs.readdirSync(path.join(pluginDir, 'languages')).find((file) => (
        file.startsWith(`expert-wage-calculator-${locale}-`) && file.endsWith('.json')
    ));
...
test('English is the source language for the JavaScript catalog and UI', () => {
    ...
    const catalog = runtime.IKExpertWage.getCatalog('1405');
    assert.equal(catalog.categories[0].label, 'General and valuation');
    assert.equal(catalog.topics[0].label, 'Valuation in all fields');
    assert.equal(runtime.ExpertWageCalculatorI18n.ui.baseFee, 'Base wage for one expert');
});
...
test('The complete Persian JavaScript catalog is loadable', () => {
    ...
    assert.equal(runtime.ExpertWageCalculatorI18n.ui.baseFee, 'دستمزد پایه تک‌نفره');
...
test('POT, PO, MO, and JavaScript translation files are included for both locales', () => {
    const required = [
        'expert-wage-calculator.pot',
        'expert-wage-calculator-en_US.po',
        'expert-wage-calculator-en_US.mo',
        'expert-wage-calculator-fa_IR.po',
        'expert-wage-calculator-fa_IR.mo'
    ];
